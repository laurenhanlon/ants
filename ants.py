    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            #Regular ant
                ...
                if insect.container and insect.container != None:
                    self.ant = insect.ant

        #not an ant, so it is a bee
        else:
            self.bees.append(insect)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        while current_exit != None:
            print('Hi')
            if current_exit.ant != None:
                if current_exit.name != 'Hive':
                    if current_exit.ant not in damaged_ants:
                        current_exit.ant.damage = current_exit.ant.damage *2
                        damaged_ants.append(current_exit.ant)
                if current_exit.ant.container and current_exit.ant.ant != None:
                    if current_exit.ant not in damaged_ants:
                        current_exit.ant.damage = current_exit.ant.damage *2
                        damaged_ants.append(current_exit.ant)
            current_exit = current_exit.exit

        current_entrance = self.place.entrance

        while current_entrance != None:
            if current_entrance.ant != None:
                if current_entrance.ant != 'AntColony':
                    if current_entrance.ant not in damaged_ants:
                        current_entrance.ant.damage = current_entrance.ant.damage *2
                        damaged_ants.append(current_entrance.ant)
                if current_entrance.ant.container and current_entrance.ant.ant != None:
                    if current_entrance.ant not in damaged_ants:
                        current_entrance.ant.damage = current_entrance.ant.damage *2
                        damaged_ants.append(current_entrance.ant)
            current_entrance = current_entrance.entrance
        self.throw_at(self.nearest_bee(colony.hive))
